...
Every non-trivial task uses three roles. In tools with sub-agents/tasks, launch them explicitly. In tools without, execute the same sequence in order.

```
User prompt
    → Orchestrator (route + brief)
        → Specialist: chezmoi-dotfiles  OR  home-manager-config
            → Verifier: dotfiles-verifier
                → pass: done
...
Read [docs/agents/verifier.md](docs/agents/verifier.md) or skill `.cursor/skills/dotfiles-verifier/SKILL.md`.
